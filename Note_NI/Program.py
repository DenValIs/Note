...
class Note_pad:
    ...
    def add_note(self):
        if os.path.isfile("Note.csv") == False: 
            headerlist = ({"name_note": [], "date_time_last_save": [], "text_note": []})
            with open("Note.csv", "w", newline="", encoding="utf-8") as file_note:
        ...
        with open("Note.csv", "a", newline="", encoding="utf-8") as note:
    ...
    def list_note(self):
        ...
        with open("Note.csv", encoding="utf-8") as note_list:
    ...
    def select_note(self):
        ...
        with open("Note.csv", encoding="utf-8") as line:
            reader = csv.DictReader(line, delimiter=";")
            for i, row in enumerate(reader):
                if row["name_note"] == name:
                    self.number_line = i
                    print("Выберете операцию с заметкой:\n1) Посмотреть заметку\n2) Изменить заметку\n3) Удалить заметку")
                    count = input()
                    if count == "1":
                        with open("Note.csv", encoding="utf-8") as csv_file:
                            csv_reader = csv.reader(csv_file)
                            rows = list(csv_reader)
                            print(rows[self.number_line + 1])
                    if count == "2":
                        with open("Note.csv", encoding="utf-8", newline='') as source, open("New_note.csv", "w", encoding="utf-8", newline='') as dest:
                        ...
                        shutil.copy2(r"New_note.csv", r"Note.csv")
                        os.remove('New_note.csv')
                    if count == "3":
                        with open("Note.csv", newline='') as source, open("New_note.csv", "w", newline='') as dest:
                            reader = csv.reader(source, delimiter=';')
                            writer = csv.writer(dest,delimiter=';')
